...
struct LakeSceneView: View {
    ...
    var body: some View {
        Canvas { context, _ in
            ...
            drawHills(context, W: W, factor: 0.12, baseY: waterlineY - 26, height: 60,
                      color: palette.hillFar, seedShift: 0)
            drawHills(context, W: W, factor: 0.28, baseY: waterlineY - 8, height: 42,
    ...
    private func drawWater(_ c: GraphicsContext, W: CGFloat, H: CGFloat, t: Double) {
        ...
        c.fill(p, with: .linearGradient(
            ...
            startPoint: CGPoint(x: 0, y: waterlineY - 14),
            endPoint: CGPoint(x: 0, y: H)))

        // Shimmer bands
        for band in 0..<4 {
            let bandY = waterlineY + CGFloat(16 + band * 26)
            ...
            for i in 0...70 {
                ...
                if vis > 0.65 {
                    let yy = bandY + CGFloat(sin(wx * 1.7 + phase) * 2.2)
            ...
            c.stroke(sp, with: .color(palette.shimmer.opacity(0.32 - Double(band) * 0.06)),
        ...
        var rp = Path()
        for i in 0..<9 {
            let yy = waterlineY + CGFloat(8 + i * 14)
            guard yy < H else { break }
            let wobble = CGFloat(sin(t * 1.3 + Double(i) * 1.1) * 7)
            let wdt = CGFloat(26 - i * 2)
    ...
    private func drawDistanceTicks(_ c: GraphicsContext, H: CGFloat) {
        ...
        while Double(m) < cameraX + visible + 10 {
            ...
            if px > -30 && px < screenSize.width + 30 && m > 0 {
                ...
                tp.addLine(to: CGPoint(x: px, y: yy + (m % 20 == 0 ? 9 : 5)))
                c.stroke(tp, with: .color(Color.white.opacity(0.35)), lineWidth: 1.4)
                if m % 20 == 0 {
                    c.draw(Text("\(m)").font(.custom("Menlo", size: 9)).foregroundColor(.white.opacity(0.5)),
                           at: CGPoint(x: px, y: yy + 17))
                }
            }
            m += 10
        }
    ...
    private func drawLaunchBank(_ c: GraphicsContext, H: CGFloat) {
        let edge = sx(-0.4)
        guard edge > -60 else { return }
        var p = Path()
        p.move(to: CGPoint(x: edge, y: sy(-0.05)))
        p.addLine(to: CGPoint(x: edge - 90, y: sy(0.9)))
        p.addLine(to: CGPoint(x: edge - 90, y: H))
        p.addLine(to: CGPoint(x: edge + 14, y: H))
        p.closeSubpath()
    ...
    private func drawFarShore(_ c: GraphicsContext, H: CGFloat, t: Double) {
        ...
        pole.addLine(to: CGPoint(x: fx, y: fy - 34))
        c.stroke(pole, with: .color(SS.ink.opacity(0.7)), lineWidth: 2)
        var flag = Path()
        let wave = CGFloat(sin(t * 2.1) * 3)
        flag.move(to: CGPoint(x: fx, y: fy - 34))
        flag.addLine(to: CGPoint(x: fx + 20 + wave, y: fy - 29))
        flag.addLine(to: CGPoint(x: fx, y: fy - 23))
        flag.closeSubpath()
    ...
    private func drawRings(_ c: GraphicsContext, t: Double) {
        for (i, ring) in engine.level.rings.enumerated() {
            ...
            c.stroke(Path(ellipseIn: CGRect(x: px - r, y: py - r, width: r * 2, height: r * 2)),
                     with: .color(color), lineWidth: 4)
            c.stroke(Path(ellipseIn: CGRect(x: px - r * 0.7, y: py - r * 0.7, width: r * 1.4, height: r * 1.4)),
                     with: .color(color.opacity(0.4)), lineWidth: 1.6)
            if collected {
                c.draw(Text("+1").font(.custom("Menlo-Bold", size: 11)).foregroundColor(SS.gold),
                       at: CGPoint(x: px, y: py - r - 10))
            }
        }
    }
    ...
    private func drawBuoys(_ c: GraphicsContext, t: Double) {
        for b in engine.level.buoys {
            ...
            ctx.stroke(mast, with: .color(SS.ink.opacity(0.6)), lineWidth: 2)
            ctx.fill(Path(ellipseIn: CGRect(x: -3, y: -bodyH * 0.95, width: 6, height: 6)),
                     with: .color(SS.gold))
        }
    }
    ...
    private func drawWood(_ c: GraphicsContext, t: Double) {
        for wd in engine.level.wood {
            ...
            c.fill(Path(roundedRect: CGRect(x: px - w / 2 + 2, y: topY + 2, width: w * 0.35, height: botY - topY - 4),
                        cornerRadius: w * 0.2),
                   with: .color(SS.hex(0x9A744E).opacity(0.8)))
            // knot
            c.fill(Path(ellipseIn: CGRect(x: px - 3, y: topY + (botY - topY) * 0.3, width: 6, height: 6)),
    ...
    private func drawGhost(_ c: GraphicsContext) {
        ...
        for (i, pt) in ghost.enumerated() {
            ...
            c.fill(Path(ellipseIn: CGRect(x: px - 2, y: py - 2, width: 4, height: 4)),
        ...
        for f in engine.fx {
            ...
            if age < 0.7 {
                ...
                for _ in 0..<n {
                    ...
                    c.fill(Path(ellipseIn: CGRect(x: dpx - 1.6, y: dpy - 1.6, width: 3.2, height: 3.2)),
                           with: .color(Color.white.opacity(alpha)))
                }
            }
            ...
            if f.kind == .perfect && age < 0.5 {
                let r = CGFloat(age) * 46 + 8
                let alpha = max(0, 0.7 - age * 1.5)
                c.stroke(Path(ellipseIn: CGRect(x: px - r, y: py - r - 6, width: r * 2, height: r * 2)),
                         with: .color(SS.gold.opacity(alpha)), lineWidth: 2.4)
            }
        }
    }
    ...
    private func drawAimPreview(_ c: GraphicsContext) {
        ...
        for i in 0..<18 {
            ...
            let r: CGFloat = 3.4 - CGFloat(i) * 0.09
        ...
        var arc = Path()
        arc.addArc(center: CGPoint(x: cx, y: cy), radius: 30,
                   startAngle: .degrees(0), endAngle: .degrees(-64), clockwise: true)
        c.stroke(arc, with: .color(Color.white.opacity(0.25)), style: StrokeStyle(lineWidth: 5, lineCap: .round))
        var arc2 = Path()
        arc2.addArc(center: CGPoint(x: cx, y: cy), radius: 30,
                    startAngle: .degrees(0), endAngle: .degrees(-64 * aim.power), clockwise: true)
        c.stroke(arc2, with: .color(SS.gold), style: StrokeStyle(lineWidth: 5, lineCap: .round))
    }
    ...
    private func drawStone(_ c: GraphicsContext) {
        ...
        if engine.phase == .flying {
            ...
            if sp > 6 {
                ...
                tp.addLine(to: CGPoint(x: -w / 2 + 4, y: 0))
    ...
    private func drawTimingRing(_ c: GraphicsContext) {
        ...
        guard px > -80 && px < screenSize.width + 80 else { return }

        // Fixed target ring
        let targetR: CGFloat = 15
        c.stroke(Path(ellipseIn: CGRect(x: px - targetR, y: py - targetR,
                                        width: targetR * 2, height: targetR * 2)),
                 with: .color(Color.white.opacity(0.9)), lineWidth: 2.6)

        // Shrinking ring: tap when it meets the target
        let shrinkR = targetR + CGFloat(progress) * 46
        ...
        c.stroke(Path(ellipseIn: CGRect(x: px - shrinkR, y: py - shrinkR,
                 ...
                 with: .color(ringColor), lineWidth: near ? 3.4 : 2.2)

        // Contact point marker on the wave
        var cm = Path()
        cm.move(to: CGPoint(x: px - 6, y: py))
        cm.addLine(to: CGPoint(x: px + 6, y: py))
        cm.move(to: CGPoint(x: px, y: py - 6))
        cm.addLine(to: CGPoint(x: px, y: py + 6))
        c.stroke(cm, with: .color(Color.white.opacity(0.8)), lineWidth: 1.4)

        if engine.tapFlashActive {
            c.fill(Path(ellipseIn: CGRect(x: px - targetR - 4, y: py - targetR - 4,
                                          width: (targetR + 4) * 2, height: (targetR + 4) * 2)),
    ...
    private func drawReedsForeground(_ c: GraphicsContext, W: CGFloat, H: CGFloat) {
        let edge = sx(-0.4)
        guard edge > -140 else { return }
        var rng = SeededRNG(seed: 777)
        for i in 0..<7 {
            let bx = edge - CGFloat(10 + i * 13) + CGFloat(rng.range(-4, 4))
            guard bx > -20 else { continue }
            let baseY = H - CGFloat(rng.range(0, 30))
            let hgt = CGFloat(rng.range(50, 110))
            let sway = CGFloat(sin(engine.time * 0.8 + Double(i) * 1.3)) * 4
            var reed = Path()
            reed.move(to: CGPoint(x: bx, y: baseY))
            reed.addQuadCurve(to: CGPoint(x: bx + sway, y: baseY - hgt),
                              control: CGPoint(x: bx - 3, y: baseY - hgt * 0.55))
            c.stroke(reed, with: .color(palette.reed), lineWidth: CGFloat(rng.range(2.2, 3.6)))
            let headR = CGFloat(rng.range(3, 5))
